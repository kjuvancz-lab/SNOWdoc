# Zurich Sales and Order Management — Order Management reference

Source: servicenow-zurich-order-management-enus.pdf | Release: zurich | Pages: 1310–1359 of 1408

Sections: Order Management reference (p1310)
...
For more information about the Training Portal and how to purchase a subscription to the
Portal, contact ServiceNow CPQ Training at training@logik.io
or your ServiceNow CPQ Sales
Representative.
...
about order types, see Order types in Sales and Order
Management.
Account
Contact
...
contracted rate. Options include:
...
Contracted price. Options include:
...
<!-- page 1323 -->
Activity and attachment fields (continued)
Field
...
the order line item. To learn more, see .
...
The priority of completing the order. Options
include:
• 1 - Critical
• 2- High
• 3 - Moderate
...
The location or site at which a product must be
delivered, provisioned, or serviced.
...
The service location changes to new location.
This is read-only check box.
It’s applicable only when the action state is
Change.
...
• Select the activity stream filter icon ( ) to
...
The priority of the order task. Options include:
• None
• 1 - Critical
• 2 - High
• 3 - Moderate
...
The data capture phase is when an agent is
creating and configuring order line items or
your instance is capturing full customer order
details from an API or through Service Bridge.
In this case, Draft is a system-assigned state.
This state is assigned by the system.
